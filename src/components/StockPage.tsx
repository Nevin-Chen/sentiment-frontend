...
const StockPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getCompanyProfile();
    getStockNews();
  }, [getStockData, getCompanyProfile, getStockNews]);
  ...
  return (
    <>
      <div className="title-container">
        <div className="company-name">{companyProfile?.companyName}</div>

        <div className="company-symbol">
          <a href={companyProfile?.website}>({companyProfile?.symbol})</a>
        </div>
        {source === "Massive" && (
          <Tooltip text="Chart data may be delayed by up to 1 day" />
        )}
      </div>
      ...
      <div>{companyProfile && <Profile companyProfile={companyProfile} />}</div>
    </>
  );
};
